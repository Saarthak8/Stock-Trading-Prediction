...
def make_prediction(tick):
    ...
    plt.figure(figsize = (16,8))
    plt.title('TRAINING INSIGHTS', fontsize= 18)
    ...
    ## Plot the prediction
    plt.figure(figsize = (16,8))
    plt.title('NEXT 30 DAYS PREDICTION', fontsize= 18)
    ...
    plt.figure(figsize = (16,8))
    plt.title('COMPLETE DATA INCLUDING PREDICTION', fontsize= 18)
    plt.plot(df3)
    plt.grid(color='b', ls = '-.', lw = 0.25)
    plt.xticks(range(0,df.shape[0],50),df['Date'].loc[::50],rotation=45)
    plt.xlabel('Date',fontsize=18)
    ...
    ## Return the 30 day predicted value
    return((scaler.inverse_transform(lst_output)[-1])[0])
